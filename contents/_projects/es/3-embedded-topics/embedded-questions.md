...
2. **¿Qué es el *keyword* *volatile* y dónde/ para qué se utiliza?**         
...
3. **¿Para que se usa la palabra reservada *static*? ¿qué pasa si se aplica a una variable? ¿qué pasa si la lleva una función?**     
...
9. **¿Qué hace la palabra *static* en una variable *global* y en una *local*?**     
   - **Variable local `static`{:.info}**: Conserva su valor entre llamadas a funciones, se inicializa una vez y persiste durante la vida del programa.    
   Ejemplo: 
   ```c 
   static int count = 0;
   ``` 
   En una función se incrementa entre llamadas.  
   - **Variable global `static`{:.info}**: Restringe la visibilidad al archivo donde se declara, evitando el enlace externo.    
   Ejemplo: 
   ```c 
   static int config = 10;
   ```
   Solo es accesible dentro del archivo.  
...
11. **¿Para que se utiliza la palabra reservada *enum*?**   
    La palabra reservada **enum** en C define un tipo enumerado, asignando nombres a un conjunto de constantes enteras para mejorar la legibilidad del código. Se usa para estados u opciones, como códigos de error o modos. 
    Ejemplo: 
    ```c 
    enum State { OFF, ON, IDLE };
    ``` 
    En proyectos con AVR define estados del dispositivo, haciendo el código más claro que usar enteros crudos.
...
37. **¿Qué es una optimización de compilación?**
...
5. **¿Qué significa que un microcontrolador sea *big endian*?**
...
6. **¿Qué significa que un procesador sea *little endian*?**
...
```
23. **¿Qué es un watchdog timer? ¿Qué funciones tiene?**    
...
27. **¿Cuales son las partes de la memoria de un sistema y qué se hace en cada una de ellas?**    
...
29. **¿Cómo se mapea un **GPIO** dentro del microcontrolador (en referencia a lenguaje C)?**      
...
9. **¿Qué significa un protocolo de tipo **Full-Duplex** ó **Half-Duplex**?**  
   **Full-Duplex (Dúplex Completo)**
   Definición: Permite la transmisión simultánea de datos en ambas direcciones. Ambos dispositivos pueden enviar y recibir datos al mismo tiempo.
   Características: 
     - Comunicación bidireccional simultánea
     - Canales separados para transmisión y recepción
