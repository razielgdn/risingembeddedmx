...
```
## Changing the Demo
The `Prog/` folder contains a demo application with a **blinking LED**. This app serves two purposes:
- To verify that firmware flashing via OpenBLT is working correctly.
- To provide a working example that follows the correct memory layout for the bootloader.
Just like the bootloader project in the `Boot/` folder, several files in the `Prog/` application must be modified to match the Bluepill Plus hardware and memory configuration.    
...
Note: The next steps—including detailed instructions for using the bootloader, performing firmware updates, and working with MicroBoot—are thoroughly explained in the project documentation available in the [razielgdn GitHub repository](https://github.com/razielgdn/black-and-blue-.pill-plus-with-openBLT/blob/main/README.md)
